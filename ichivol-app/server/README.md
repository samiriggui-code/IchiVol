...
- **`src/notifications/digest.ts`** -- une fois par `DIGEST_INTERVAL_MS` (24h par défaut, premier envoi ~2 min après boot), assemble un **briefing desk** via `buildBriefing.ts` : equity / P&L / cash / livre ouvert / tape 24 h / ShadowBroker / preuve edge backtest. Email React Email brandé (`DigestEmail.tsx`) + **PDF joint** (`DigestPdf.tsx` via `@react-pdf/renderer`). Sujet du type `IchiVol Briefing — YYYY-MM-DD · equity N €`. Lien app via `APP_URL` (défaut prod).

Les deux créent une notification in-app (`kind: system_alert | system_digest`, visibles dans la cloche header comme n'importe quelle autre) **et**, si `SMTP_HOST`/`SMTP_USER`/`SMTP_PASSWORD` sont configurés, un email via `src/notifications/mailer.ts` (nodemailer, pièces jointes supportées). Sans config SMTP, `sendMail()` log un avertissement une fois et retourne `false` sans jamais lever d'exception -- les notifications in-app continuent de fonctionner dans tous les cas, l'email est un bonus optionnel.

Smoke test : `npx tsx scripts/smoke-e7-notifications.ts` -- health check, rendu email briefing + PDF non vide, `sendMail` ne throw jamais.
