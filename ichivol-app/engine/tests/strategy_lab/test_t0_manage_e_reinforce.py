...
def test_parse_reinforce_default_is_tighten_stop():
    rs = parse_ruleset(
        {
            "id": "rf",
            "direction": "LONG",
...
def test_parse_reinforce_roundtrip_explicit_reduce_qty():
    ...
    assert rs.exit.reinforce.risk_policy == "reduce_qty"
...
def test_parse_rejects_partial_tp_with_reinforce():
    with pytest.raises(ValueError, match="partial_tp and reinforce"):
        parse_ruleset(
            {
                "id": "combo",
                "direction": "LONG",
                ...
                "target_atr": 3.0,
                "exit": {
                    "partial_tp": [{"r_multiple": 1.0, "fraction": 0.5}],
                    "reinforce": {
                        "conditions": {"tk_cross_bullish": True},
                        "add_fraction": 0.5,
                    },
...
def test_open_risk_signed_stop_beyond_avg_is_zero():
    # LONG: stop above avg → no adverse distance left → 0 risk
    assert open_risk(Direction.LONG, avg_entry=100.0, stop=105.0, qty=2.0) == 0.0
    # SHORT: stop below avg → 0 risk
    assert open_risk(Direction.SHORT, avg_entry=100.0, stop=95.0, qty=2.0) == 0.0
    # LONG normal
    assert open_risk(Direction.LONG, 100.0, 90.0, 1.0) == pytest.approx(10.0)
    # SHORT normal
    assert open_risk(Direction.SHORT, 100.0, 110.0, 1.0) == pytest.approx(10.0)
...
def test_open_risk_and_clamp_reduce_qty():
    ...
    assert open_risk(Direction.LONG, avg, stop, qty) == pytest.approx(10.0, abs=1e-6)


def test_tighten_stop_adds_in_profit():
    """Default path: add above avg with fixed stop → stop tightens, full add kept."""
    actual, avg, stop, qty, clamped = apply_reinforce_add(
        direction=Direction.LONG,
        avg_entry=100.0,
        qty=1.0,
    ...
    assert avg == pytest.approx((100 + 60) / 1.5)
    assert clamped is True
    assert stop > 90.0
    assert open_risk(Direction.LONG, avg, stop, qty) == pytest.approx(10.0, abs=1e-9)


def test_reduce_qty_blocks_add_above_avg_with_fixed_stop():
    """Structural: reduce_qty + fixed stop + add above avg → 0 (not MTM quirk)."""
    actual, avg, stop, qty, clamped = apply_reinforce_add(
        direction=Direction.LONG,
        avg_entry=100.0,
        qty=1.0,
...
def test_reinforce_then_target_records_add_and_entry_vwap():
    """Entry 100 stop 90. Default tighten_stop add 0.5 @ 105. Exit target 120."""
    candles = [
        _c(0, 100, 100, 100, 100),
        _c(1, 100, 100, 99, 100),
        _c(2, 100, 106, 99, 105),
        _c(3, 105, 121, 104, 120),
    ]
    ...
    assert d.reinforce_adds[0].clamped is True  # stop moved
    ...
    assert d.reinforce_adds[0].open_risk_after == pytest.approx(
        open_risk(Direction.LONG, 100.0, 90.0, 1.0), abs=1e-6
    )
