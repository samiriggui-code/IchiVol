/**
 * UI-P2 smoke — Portefeuille + Opportunités, desktop + mobile 390.
...
async function main() {
  ...
  for (const [route, file, needles] of [
    ['portefeuille', '01-portefeuille-desktop', ['Capital', 'Risk Kernel', 'Allocation', 'Concentration']],
    ['opportunites', '02-opportunites-desktop', ['De l’observation', "De l'observation", 'Opportunités']],
  ]) {
    ...
    report.pages[file] = r
    console.log(`[${file}]`, r.pass && has ? 'PASS' : 'FAIL', r.titles.slice(0, 8), r.apiErrors)
  }

  // p2 checks on opportunités
  const p2 = await page.evaluate(() => {
    const text = document.body.innerText
    return {
      hasRibbon: /De l.observation à la décision/.test(text),
      hasSegment: /No trade|Surveillance/.test(text),
      hasArmedDemo: /\bARMED\b|\bTRIGGERED\b/.test(text),
      hasWhy: /Pourquoi /.test(text),
    }
  })
  const p2Ok = p2.hasRibbon && p2.hasSegment && !p2.hasArmedDemo
  if (!p2Ok) report.pass = false
  report.pages.p2Checks = p2
  console.log('[p2 checks]', p2Ok ? 'PASS' : 'FAIL', p2)

  await page.setViewport({ width: 390, height: 844, isMobile: true, hasTouch: true })
  for (const [route, file] of [
    ['portefeuille', '03-portefeuille-mobile-390'],
    ['opportunites', '04-opportunites-mobile-390'],
  ]) {
    const r = await collectPage(page, route, file)
  ...
  await browser.close()
  console.log('\n=== UI-P2 SUMMARY ===', report.pass ? 'PASS' : 'FAIL')
