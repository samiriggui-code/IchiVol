  ...
  return (
    <div className="chip-row">
      {codes.map((code) => (
        <span
          key={code}
          className={`sig-chip${tone === 'risk' ? ' risk-chip' : ''}`}
          title={code}
        >
          {labelReason(code)}
        </span>
...
/**
 * Signal Evidence Card — primary analysis summary.
 * Only displays values returned by the engine (no hardcoded stats).
 */
  ...
  return (
    <article className="signal-evidence-card" aria-label="Signal Evidence Card">
      ...
      <footer className="sec-foot muted">
        {ev?.evidence_engine_version ?? 'evidence'} ·{' '}
        {detail.strategy_version}
        {ev?.feature_version ? ` · ${ev.feature_version}` : ''}
        {' · '}
        conf. combiner {num(detail.confidence * 100, 0)}% (MVP, pas une preuve)
      </footer>
    </article>
  )
}
