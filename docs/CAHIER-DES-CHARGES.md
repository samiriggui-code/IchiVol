...
- [x] **T6 — AuditReport** (2026-09-23) — post-outcome ; hypothèses `proposed` only. PR #36.
- [x] **T7 — Monte Carlo / risk of ruin** (2026-09-23) — bootstrap returns nets ; min_trades. PR #37.
- [x] **T3d — propose ruleset edit** (2026-09-23) — catalogue conditions + patch validé (`proposed` only ; pas de chat UI). PR #39.
...
- [ ] **T-CYCLE — Cycle / Spectral Engine (dimension TEMPS)** — période dominante, phase, stabilité, régime TREND/CYCLE/TRANSITION/NOISE. **V0 observe-only LIVRÉ** (2026-09-26) : FFT + Hilbert + ACF → `CycleState` ; `GET /cycle/{symbol}` + `/study` ; agent `get_cycle_state` / `run_cycle_study` ; walk-forward + null models ; **ne change aucune gate / paper / confidence**. Checkbox reste ouverte : promotion décision **uniquement** après multi-symbol OOS + ablation A–G. EMD/EEMD = Lab only. Ref : [`CYCLE_ENGINE_AUDIT.md`](./CYCLE_ENGINE_AUDIT.md).
- [ ] **T-CI-BRIEF — Chart Intelligence Briefing** (période + packs calques + caméra) — **chantier ouvert 2026-09-26**. Sur fiches Décisions/Position uniquement (pas Marché). V0 = directeur heuristique (`defaultBriefing`) + UI `BriefingControls` + `ChartCamera` follow/fit. Eve-piloted briefing = suite après revue Claude. Contrat `ChartObject` only, aucun niveau inventé. Ref : [`HANDOFF-CURSOR-V3.md`](./HANDOFF-CURSOR-V3.md) entrée du jour.  
